...
class ResourceAutoscaler(CeleryAutoscaler):
    ...
    def _maybe_scale(self, req=None):
        ...
        debug("Autoscale: %s processes and %s reserved tasks" % (proc_count, task_count))
        ...
        if proc_count > max_procs:
            debug("Requesting scale down by %s" % (proc_count - max_procs))
        ...
        if proc_count < min_procs:
            debug("Requesting scale up by %s" % (min_procs - proc_count))
